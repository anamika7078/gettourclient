...
export default function UserHolidaysEnquiries() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    let alive = true;
    setLoading(true);
    fetch(`${API_BASE}/api/holiday-enquiries`)
      .then((res) => res.json())
      .then((payload) => {
        if (!alive) return;
      ...
      .catch((e) => console.error("Failed to load holiday enquiries:", e))
      ...
      <div className="max-w-5xl mx-auto p-4">
        <div className="bg-white rounded-lg shadow p-6 text-center">
          <p className="text-gray-700">
            Please login to see your holiday enquiries.
          </p>
          <button
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">My Holiday Enquiries</h1>
          <p className="text-sm text-gray-500">
            Your submitted holiday enquiry forms.
          </p>
        </div>
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          placeholder="Search by package, destination, date, or id..."
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-orange-50 sticky top-0 z-10">
              <tr className="text-left text-gray-700">
                ...
                <th className="px-3 py-2 whitespace-nowrap">Destination</th>
              ...
              {loading ? (
                ...
                filtered.map((e) => (
                  <tr key={e.id} className="border-t hover:bg-orange-50/40">
                    ...
                    <td className="px-3 py-2">{e.package_title || "—"}</td>
                    <td className="px-3 py-2">{e.destination || "—"}</td>
                    <td className="px-3 py-2">{e.travel_date || "—"}</td>
                    <td className="px-3 py-2">{e.adults ?? 0}</td>
                    <td className="px-3 py-2">{e.children ?? 0}</td>
                    <td className="px-3 py-2">
                      {e.price != null ? (
                        <span>
                          {convertAndFormat(e.price)}{" "}
                ...
                <li key={e.id} className="p-4">
                  ...
                  <div className="mt-3 space-y-1 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-right ml-2">{e.adults ?? 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Children</span>
                      <span className="text-right ml-2">{e.children ?? 0}</span>
                    </div>
                    <div className="flex justify-between">
              ...
              <div className="space-y-2">
                ...
                <div className="bg-gray-50 rounded p-3 space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{selected.flight_booked || "—"}</span>
                  </div>
                  <div className="flex justify-between">
